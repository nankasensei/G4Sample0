...
public:
	COMPORT( LPCWSTR PORT );
	COMPORT(LPCWSTR PORT, DWORD Speed);
	operator BOOL( void )
	{
...
COMPORT::COMPORT(LPCWSTR PORT)
{
	...
	const char* c = b;
	//
	hCom	= CreateFile(c, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, NULL, NULL);
	loadDefalutState();
	setState();
}

COMPORT::COMPORT(LPCWSTR PORT, DWORD Speed)
{
	//
	const WCHAR* wc = PORT;
	_bstr_t b(wc);
